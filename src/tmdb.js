...
async function fetchSeasonVideos(auth, tmdbId, season, lang, poster) {
  try {
    ...
    return eps.map(ep => ({
      ...
      thumbnail: ep.still_path ? `${TMDB_IMAGE}/w300${ep.still_path}` : (season.poster_path ? `${TMDB_IMAGE}/w300${season.poster_path}` : poster),
    ...
    return [{
      ...
      season:  season.season_number,
      episode: 1,
      poster:  season.poster_path ? `${TMDB_IMAGE}/w500${season.poster_path}` : poster,
...
async function getMetadata(apiKey, tmdbId, type, lang = 'pt-BR') {
  ...
  if (type === 'movie') {
    ...
    // Buscar episódios de todas as temporadas em paralelo
